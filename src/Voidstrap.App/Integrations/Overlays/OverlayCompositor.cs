...
namespace Voidstrap.Integrations.Overlays
{
    ...
    {
        private const string ClassName = "VoidstrapOverlayCompositor";
        private const string CaptureWindowName = "Voidstrap Game Output";
        private const string LOG_IDENT = "Overlays";
        private const uint CreateWaitableTimerHighResolution = 2;
        private const uint TimerAllAccess = 0x1F0003;

        [LibraryImport("kernel32.dll", SetLastError = true, StringMarshalling = StringMarshalling.Utf16)]
        private static partial IntPtr CreateWaitableTimerExW(IntPtr timerAttributes, string? timerName, uint flags, uint desiredAccess);

        [LibraryImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        private static partial bool SetWaitableTimer(IntPtr timer, ref long dueTime, int period, IntPtr completionRoutine, IntPtr argument, [MarshalAs(UnmanagedType.Bool)] bool resume);
		...
		private HomepageVideoDecoder? _homepageVideo;
        ...
        private readonly MotionBlurOverlay _motionBlur = new MotionBlurOverlay();
        private readonly FrameGenPipeline _fg = new FrameGenPipeline();
        private bool _riAttached;
        private long _riAttachRetryAt;
        private bool _aaAttached;
        private bool _mbAttached;
        private long _mbAttachRetryAt;
        ...
        private void CreateDevice()
        {
            ...
            try
            {
                if (chosen != null)
                {
                    CreateD3DDevice(chosen, DriverType.Unknown).CheckError();
            ...
            try
            {
                CreateD3DDevice(null, DriverType.Hardware).CheckError();
        ...
        private SharpGen.Runtime.Result CreateD3DDevice(IDXGIAdapter? adapter, DriverType driverType)
        {
            SharpGen.Runtime.Result result = D3D11.D3D11CreateDevice(adapter, driverType, DeviceCreationFlags.BgraSupport | DeviceCreationFlags.VideoSupport, _featureLevels, out _device, out _context);
            if (result.Success)
                return result;
            _context?.Dispose();
            _device?.Dispose();
            return D3D11.D3D11CreateDevice(adapter, driverType, DeviceCreationFlags.BgraSupport, _featureLevels, out _device, out _context);
        }
        ...
        private void SetRecordingVisibility(bool visible)
        {
            ...
            App.Logger.WriteLine(LOG_IDENT, visible
                ? "OBS output ready as Voidstrap Game Output"
        ...
        private void CreateSizedResources()
        {
            ...
            if (_homepageMedia != null && _homepageMediaWidth > 0 && _homepageMediaWidth < _width && _homepageMediaHeight < _height)
            {
        ...
        private void FollowRoblox()
        {
            ...
            if (sizeChanged)
            {
                ...
                _backBufferTex?.Dispose();
                _backBufferTex = null;
                _swapChain!.ResizeBuffers(4, (uint)_width, (uint)_height, Format.B8G8R8A8_UNorm, _swapChainFlags);
                CreateBackBufferRtv();
                CreateSizedResources();
                _refreshHz = QueryRefreshHz();
                if (_wgc == null)
		...
		private bool TryHomepageIdlePresent()
		{
			...
			if (now < _homepageIdleNextMs)
				return false;
			_homepageIdleNextMs = now + 1000.0 / Math.Max(30.0, _refreshHz);
		...
		private bool UpdateHomepageMedia()
		{
			try
			{
				...
				if (!string.Equals(resolved, _homepageMediaPath, StringComparison.OrdinalIgnoreCase))
				{
					...
					{
						_homepageVideo = TryCreateHomepageVideo(resolved);
						if (_homepageVideo == null)
							_homepageMedia = new HomepageBackgroundMedia(resolved, _refreshHz, _width, _height);
					}
				}
				if (_homepageVideo != null)
				{
					if (!_homepageVideo.Failed)
					{
						if (_homepageVideo.Update(_context!))
							_homepageMediaUploaded = true;
						return _homepageVideo.HasFrame;
					}
					string failedPath = _homepageMediaPath;
					ReleaseHomepageMedia();
					_homepageMediaPath = failedPath;
					_homepageMedia = new HomepageBackgroundMedia(failedPath, _refreshHz, _width, _height);
				}
		...
		private HomepageVideoDecoder? TryCreateHomepageVideo(string path)
		{
			if (Voidstrap.Utility.Platform.IsLinux)
				return null;
			string extension = Path.GetExtension(path).ToLowerInvariant();
			if (extension is not (".mp4" or ".m4v" or ".webm" or ".avi" or ".mov" or ".wmv" or ".mpeg" or ".mpg" or ".mkv"))
				return null;
			try
			{
				HomepageVideoDecoder video = new HomepageVideoDecoder(_device!, path);
				_homepageMediaSrv = video.Srv;
				_homepageMediaWidth = video.Width;
				_homepageMediaHeight = video.Height;
				App.Logger.WriteLine(LOG_IDENT, $"Homepage background video is decoding on the GPU at {video.Width}x{video.Height}");
				return video;
			}
			catch (Exception ex)
			{
				App.Logger.WriteLine(LOG_IDENT, "GPU video decoding is unavailable for the homepage background, using the software player: " + ex.Message);
				return null;
			}
		}
        ...
        private void SyncStages(bool riEnabled, bool riOn, bool aaOn, bool mbOn, bool fgOn)
        {
            if (mbOn && !_mbAttached && Environment.TickCount64 >= _mbAttachRetryAt && Voidstrap.Integrations.MotionBlur.MotionBlurOverlay.EnsurePrepared())
            {
                try { _motionBlur.AttachExternal(_device!, _context!, _width, _height); _mbAttached = true; }
                catch (Exception ex)
                {
                    App.Logger.WriteException("OverlayCompositor::AttachMotionBlur", ex);
                    _mbAttachRetryAt = Environment.TickCount64 + 10000;
                }
            }
            else if (!mbOn && _mbAttached)
            {
                _motionBlur.DisposeExternal();
                _mbAttached = false;
            }

            if (riEnabled && !_riAttached && Environment.TickCount64 >= _riAttachRetryAt)
            {
                try
                {
                    _rishade.AttachExternal(_device!, _context!, _width, _height);
                    _riAttached = true;
                }
                catch (Exception ex)
                {
                    App.Logger.WriteException("OverlayCompositor::AttachRiShade", ex);
                    _rishade.DisposeExternal(stopDepth: false);
                    _riAttachRetryAt = Environment.TickCount64 + 10000;
                }
            }
            else if (!riEnabled && _riAttached)
            {
                _rishade.DisposeExternal(stopDepth: true);
                _riAttached = false;
        ...
        private void RenderFrame(CancellationToken token)
        {
            ...
            int mbStrength = MotionBlurSettings.StrengthIndex;
            bool mbOn = mbStrength > 0 && OverlayHub.InGame;
            bool fgConfigured = fgMode > 0 && OverlayHub.InGame;
            bool fgOn = fgConfigured && Environment.TickCount64 >= Volatile.Read(ref _fgHoldUntilTick);
            SyncStages(riEnabled, riOn, aaOn, mbOn, fgConfigured);
            ...
            if (!fresh)
            {
                ...
                {
                    if (_wgc != null)
                        _wgc.WaitForFrame(_fgGenerating ? 1 : _homepageMedia?.IsAnimated == true || _homepageVideo != null ? 4 : 20);
                }
                return;
            }
            _rawValid = true;
            ...
            if (riOn && _riAttached)
            {
                try
                {
                    _rishade.RenderInto(_rawSrv!, _stageRtv[0]!, _width, _height);
            ...
            if (mbOn && _mbAttached)
            {
                try
                {
                    int mbStage = ReferenceEquals(composited, _stageSrv[0]) ? 1 : 0;
                    if (_stageRtv[mbStage] != null && _stageSrv[mbStage] != null)
                    {
                        _motionBlur.RenderInto(composited, _stageRtv[mbStage]!, _width, _height);
                        composited = _stageSrv[mbStage]!;
                    }
                }
                catch (Exception ex)
                {
                    App.Logger.WriteException("OverlayCompositor::MotionBlurStage", ex);
                    _motionBlur.DisposeExternal();
                    _mbAttached = false;
                }
            }
        ...
        private void ComposeHomepage(ID3D11RenderTargetView target, ID3D11ShaderResourceView source, bool hasMedia)
        {
            ...
            _context!.UpdateSubresource(new FrameGenPipelineParams
            {
                ...
                Interp = color,
                Extra = new Vector4(sourceKeys.AdjustedOriginal.Normalized, 0f),
                Extra2 = new Vector4(sourceKeys.AdjustedSober.Normalized, 0f),
            }, _cbuffer!);
        ...
        {
            if (_backBufferRtv == null || !OverlayCrosshair.CanComposite())
                return;
            try
            {
                double nowMs = _clock.Elapsed.TotalMilliseconds;
        ...
        {
            bool enabled = FrameGenSettings.ModeIndex > 0 && OverlayHub.InGame;
        ...
        private void PaceWait(double untilMs, CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                if (remaining > 0.65 && _paceTimer != IntPtr.Zero)
                {
                    ...
                    {
                        _ = Interop.WaitForSingleObject(_paceTimer, (uint)Math.Clamp(Math.Ceiling(remaining + 1.0), 1.0, 1000.0));
                        continue;
                    }
                }
                if (remaining > 1.6)
